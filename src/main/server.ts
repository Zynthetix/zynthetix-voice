...
export function startServer(store: {
  ...
  // Dashboard index — in dev redirect to Vite, in production serve built file
  app.get('/', (_req, res) => {
    if (isDev) {
      return res.redirect('http://localhost:5173/src/dashboard/index.html');
    }
    res.sendFile(path.join(rendererPath, 'src/dashboard/index.html'));
  });
  ...
  app.post('/api/snippets', (req, res) => {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: 'trigger must be a non-empty string ≤ 100 chars' });
    if (
      !expansion ||
      typeof expansion !== 'string' ||
      expansion.trim().length === 0 ||
      expansion.length > 5000
    )
      return res
        .status(400)
        .json({ error: 'expansion must be a non-empty string ≤ 5000 chars' });
    insertSnippet(trigger.trim(), expansion);
    res.json({ ok: true });
  });
  app.put('/api/snippets/:id', (req, res) => {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: 'expansion must be a non-empty string ≤ 5000 chars' });
    updateSnippet(Number(req.params.id), trigger.trim(), expansion);
    res.json({ ok: true });
  });
  app.delete('/api/snippets/:id', (req, res) => {
    deleteSnippet(Number(req.params.id));
    res.json({ ok: true });
  });
  ...
  server.on('error', (err: NodeJS.ErrnoException) => {
    if (err.code === 'EADDRINUSE') {
      console.error(
        `[server] Port ${DASHBOARD_PORT} already in use — dashboard will not be available.`
      );
    } else {
